...
const HELPERS_STRINGS = {
  en: {
    ...
    textDebs: 'Text Debs',
    tellHerThoughts: 'I need to tell her my thoughts',
...
function renderChapter34Desktop(time, date) {
  ...
  switch (state.chapter34Stage) {
    case 'debs_hacked_chat_pending':
      notif = { iconClass: 'friends', icon: '💬', app: 'MESSAGE', title: HT('textDebs'), body: HT('tellHerThoughts'), goto: 'debs_hacked_chat' };
      break;
    ...
    case 'worried_chat_pending':
      notif = { iconClass: 'friends', icon: '💬', app: 'MESSAGE', title: HT('textDebs'), body: HT('tellHerThoughts'), goto: 'debs_worried_chat' };
      break;
...
function renderChatApp(config) {
  ...
  let chatBackBtn = document.getElementById('chat-back-btn');
  ...
  const newBackBtn = chatBackBtn.cloneNode(true);
  chatBackBtn.parentNode.replaceChild(newBackBtn, chatBackBtn);
  chatInput = newInput;
  chatSend = newSend;
  chatBackBtn = newBackBtn;
  // 左上角返回按钮不用 data-goto（那样无论对话是否播完都会直接跳走，行为一律相同）：
  // 对话还没播完时点它 = 中途退出，算"未完成"，回到桌面后还会再推送一次这条消息通知；
  // 对话已经播完时点它 = 等同于点击底部"go back"，要走config.onEnd（很多对话靠onEnd设置状态机/跳转下一步）。
  chatBackBtn.addEventListener('click', () => {
    if (!active) {
      if (config.onEnd) config.onEnd();
      else goToNode(config.backTarget);
    } else {
      goToNode(config.backTarget);
    }
  });
