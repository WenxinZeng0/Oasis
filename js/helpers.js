...
function renderDesktop(config) {
  ...
  document.getElementById('desktop-date').textContent = config.date || 'Thursday, November 5';
  ...
  const calendarAttr = config.showCalendarApp
    ...
    : `data-toast="Nothing new here."`;
  appGrid.innerHTML = `
    <div class="app-icon-wrap"><div class="app-icon icon-cal" ${calendarAttr}>📅</div><div class="app-label">Calendar</div></div>
    <div class="app-icon-wrap"><div class="app-icon icon-notes" data-toast="No new notes.">📝</div><div class="app-label">Notes</div></div>
    <div class="app-icon-wrap"><div class="app-icon icon-store" data-toast="Welcome back to the store.">🛍️</div><div class="app-label">Store</div></div>
    <div class="app-icon-wrap"><div class="app-icon icon-settings" data-toast="Nothing to adjust right now.">⚙️</div><div class="app-label">Settings</div></div>
    ${oasisIconHtml}
  `;
  ...
  dock.innerHTML = `
    <div class="app-icon-wrap"><div class="app-icon icon-mail" ${dg.mail ? `data-goto="${dg.mail}"` : `data-toast="No new updates right now."`}>✉️</div></div>
    <div class="app-icon-wrap"><div class="app-icon icon-friends" ${dg.friends ? `data-goto="${dg.friends}"` : `data-toast="No Updates Right Now"`}>👥</div></div>
    <div class="app-icon-wrap"><div class="app-icon icon-music" ${dg.music ? `data-goto="${dg.music}"` : `data-toast="No new updates right now."`}>🎵</div></div>
    <div class="app-icon-wrap"><div class="app-icon icon-photos" ${dg.photos ? `data-goto="${dg.photos}"` : `data-toast="No new photos."`}>🖼️</div></div>
  `;
}
...
function renderChapter1Desktop(time, date) {
  ...
  if (!state.mailNotifSeen) {
    notif = {
      ...
      title: 'Your VOCALIST has shipped!',
      body: 'Your new speaker set is on its way. Tap to view details.',
      goto: 'mail_vocalist'
    };
  } else if (!state.friendsNotifConsumed) {
    notif = {
      iconClass: 'friends', icon: '👥', app: 'FRIENDS',
      title: 'Shawn921 posted a new story',
      body: "Tap to see what Shawn's been up to.",
      goto: 'friends_feed'
    };
...
function renderChapter34Desktop(time, date) {
  ...
  switch (state.chapter34Stage) {
    case 'debs_hacked_chat_pending':
      notif = { iconClass: 'friends', icon: '💬', app: 'MESSAGE', title: 'Text Debs. I need to tell her my thoughts.', body: 'Tap to view.', goto: 'debs_hacked_chat' };
      break;
    case 'call_debra_ready':
      notif = { iconClass: 'friends', icon: '📞', app: 'PHONE', title: 'Try calling Debs', body: 'She might pick up.', goto: 'calling_debra' };
      break;
    case 'worried_chat_pending':
      notif = { iconClass: 'friends', icon: '💬', app: 'MESSAGE', title: 'Text Debs. I need to tell her my thoughts.', body: 'Tap to view.', goto: 'debs_worried_chat' };
      break;
    case 'pavlov1_pending':
      notif = { iconClass: 'mail', icon: '✉️', app: 'MAIL', title: 'New message received', body: 'Tap to view.', goto: 'pavlov_email_1' };
      dockGoto = { friends: 'friends_feed', mail: 'pavlov_email_1' };
      break;
    case 'pavlov3_pending':
      notif = { iconClass: 'mail', icon: '✉️', app: 'MAIL', title: 'New message received', body: 'Tap to view.', goto: 'pavlov_email_3' };
      dockGoto = { friends: 'friends_feed', mail: 'pavlov_email_3' };
      break;
    case 'oasis_notice_pending':
      notif = { iconClass: 'oasis', icon: '⚠️', app: 'OASIS', title: 'Anomaly Alert: We are investigating capsule failures…', body: 'Tap to view.', goto: 'oasis_notice' };
      break;
...
function renderMailApp(mailList, realMailId, backTarget) {
  ...
  function paintList() {
    ...
    list.innerHTML = '<div class="mail-list-header">Inbox</div>' + mailList.map(m => `
...
function renderChatApp(config) {
      ...
      optRow.querySelectorAll('.chat-option-btn').forEach(btn => {
        btn.addEventListener('click', () => {
          ...
          if (opt.isCorrect) {
            ...
            showToast(opt.wrongFeedback || 'Try thinking about it again…');
          }
        });
      });
      return true;
  ...
  function end() {
    ...
    endBar.innerHTML = `That's the end of the conversation — <span class="chat-endbar-link" id="chat-endbar-gobtn">go back</span>`;
...
function renderProfileFeed(config) {
  ...
  backBtn.textContent = config.backLabel || '‹ Friends';
  ...
  header.innerHTML = `
    ...
    <div class="profile-actions">
      ${config.messageTarget
        ? `<div class="profile-btn message" data-goto="${config.messageTarget}">Message</div>`
        : `<div class="profile-btn message" data-toast="${config.messageDisabledToast || 'Message unavailable right now.'}" style="opacity:0.5;">Message</div>`}
      <div class="profile-btn" data-toast="Friend request already accepted.">Friends</div>
    </div>
  `;
  ...
  feed.innerHTML = config.posts.map((p, i) => `
    <div class="post profile-feed-post ${p.goto ? 'clickable-post' : ''}" data-postidx="${i}">
      ...
      <div class="post-likes">${p.likes != null ? `❤️ ${p.likes} likes` : ''}</div>
...
function renderNarrative(config) {
  ...
  wrap.innerHTML = `
    ...
    <div class="narrative-continue-btn" data-goto="${config.goto}">${config.buttonLabel || 'Continue'}</div>
  `;
}
...
function renderOasisLogin(config) {
  ...
  wrap.innerHTML = `
    <div class="oasis-login-wrap">
      ...
      <div class="oasis-login-prompt">${config.promptText || 'Due to long inactivity, please log in again for safety.'}</div>
      ...
      <div class="oasis-login-systemnote">System: Your behavior in entering personal details help our verification of biometric consistency. Neural resonance patterns analysis in progress.</div>
      <div class="oasis-login-error" id="oasis-login-error"></div>
      <div class="oasis-login-btn" id="oasis-login-submit">Log In</div>
    </div>
  `;
...
function runDebraVerificationModal(onComplete) {
  ...
  modal.innerHTML = `
    <div class="oasis-stress-modal-box verify-box">
      ...
      <div class="verify-final hide" id="verify-final">Access Approved: <span>Authorized by Debra.</span></div>
    </div>
  `;
  document.body.appendChild(modal);

  const lines = [
    'Security Intervention: Identity conflict detected. User does not match entered profile. Analyzing neural resonance signature…',
    'Biological Verification: Neural pattern mismatch confirmed. Initiating Kinship protocol. Scanning shared gut-microbiome markers in Database…',
    "Match Found: Blood Relative confirmed. Access permissions granted via Family Wellness Agreement Section 9.4.",
    "Authorization Pending… Cross-account access request sent to paired device: 'Debs_Phone'. Standing by for user consent…"
  ];
...
function renderOasisDashboard(config) {
    ...
    <div class="oasis-deeplog-row ${l.flagged ? 'flagged' : ''} ${l.flagged && config.flaggedRowGoto ? 'clickable' : ''}" ${l.flagged && config.flaggedRowGoto ? `data-goto-log="${config.flaggedRowGoto}"` : ''}>
      ...
      ${l.flagged
        ? (config.flaggedRowGoto
            ? `<span class="oasis-deeplog-fail-label">${l.statusLabel || 'FAILED'}</span>`
  ...
  const stressSectionHtml = config.hideStressTest ? '' : `
    <div class="oasis-section">
      <div class="oasis-section-title">Stress Resilience Test</div>
      <div class="oasis-stress-row">
        <span style="font-size:12.5px; color:var(--ios-text-secondary);">Current Resilience</span>
        <span class="oasis-stress-badge">${config.stressLevel}</span>
      </div>
      <div class="oasis-stress-btn" id="oasis-stress-btn">Initiate Resilience Stress-Test</div>
    </div>
  `;
  ...
  wrap.innerHTML = `
    <div class="oasis-dash-header">
      ...
      <div class="oasis-status-row"><span><span class="oasis-status-dot"></span>Status</span><span class="v active">ACTIVE | Sync: ${config.status.sync}</span></div>
      <div class="oasis-status-row"><span>Next Calibration</span><span class="v">${config.status.nextCalibration}</span></div>
    </div>

    ${config.viewingName ? `<div class="oasis-viewing-banner">Viewing: ${config.viewingName}</div>` : ''}

    <div class="oasis-section">
      <div class="oasis-section-title">Emotional Stability Index</div>
      <div class="oasis-ring-wrap">
        ${ringSvg(config.stabilityIndex)}
        <div><div class="oasis-ring-num">${config.stabilityIndex}/100</div><div class="oasis-ring-label">${config.ringLabel || 'Currently stable'}</div></div>
      </div>
    </div>

    <div class="oasis-section">
      <div class="oasis-section-title">Automated Intervention Log</div>
      ${interventionLogHtml}
    </div>

    <div class="oasis-section">
      <div class="oasis-section-title">Neuro-Frequency Plotter — Last 24H</div>
      <div class="oasis-wave-box">${waveSvg()}</div>
      <div class="oasis-deeplogs-toggle" id="oasis-deeplogs-toggle">View Detailed Historical Data ▾</div>
    ...
    <div class="oasis-section oasis-footer-section">
      <div class="oasis-terms-link" id="oasis-terms-link">Our Terms</div>
      <div class="oasis-signout-btn" id="oasis-signout-btn">Sign Out</div>
    </div>
  `;
  ...
  document.getElementById('oasis-deeplogs-toggle').addEventListener('click', () => {
    ...
    document.getElementById('oasis-deeplogs-toggle').textContent = isHidden ? 'Hide Detailed Historical Data ▴' : 'View Detailed Historical Data ▾';
  });
...
function runStressTest(test) {
  ...
  modal.innerHTML = `
    <div class="oasis-stress-modal-box">
      <div class="oasis-stress-hr" id="stress-hr-text">Heart rate: 72 bpm</div>
      ...
      <div class="oasis-stress-answer-row">
        ...
        <div class="oasis-stress-submit" id="stress-submit-btn">Submit</div>
      </div>
  ...
  const hrText = document.getElementById('stress-hr-text');
  const statusText = document.getElementById('stress-status-text');
  let hr = 72;
  const hrInterval = setInterval(() => {
    hr += Math.floor(Math.random() * 6) + 3;
    hrText.textContent = 'Heart rate: ' + hr + ' bpm — increasing...';
    if (hr > 110) {
      clearInterval(hrInterval);
      hrText.textContent = 'Heart rate: ' + hr + ' bpm';
      statusText.textContent = 'Releasing calming pulse...';
      setTimeout(() => {
        let calmHr = hr;
        const calmInterval = setInterval(() => {
          calmHr -= 4;
          hrText.textContent = 'Heart rate: ' + Math.max(calmHr, 68) + ' bpm — stabilizing';
          if (calmHr <= 68) {
            clearInterval(calmInterval);
            statusText.textContent = 'Baseline restored. You feel... calm.';
          }
        }, 200);
      }, 700);
    }
  ...
  document.getElementById('stress-submit-btn').addEventListener('click', () => {
    ...
    statusText.textContent = (val === test.answer) ? (test.correctFeedback || 'Correct.') : (test.wrongFeedback || 'Incorrect.');
...
function renderEnding(config) {
  ...
  wrap.innerHTML = `
    ...
    <div class="ending-btns">
      ${config.showCheckpointReturn ? `<div class="ending-btn" id="ending-cp-return">Return to a Previous Choice</div>` : ''}
      <div class="ending-btn" id="ending-collection">View Ending Collection</div>
      <div class="ending-btn primary" id="ending-restart">Restart the Game</div>
    </div>
  `;
...
function renderCalendar(config) {
    ...
    return `
      <div class="cal-bday-card">
        ...
        <div class="cal-bday-countdown">
          <div class="cal-bday-days ${days <= 7 ? 'soon' : ''}">${days === 0 ? 'Today!' : days}</div>
          ${days === 0 ? '' : `<div class="cal-bday-unit">days left</div>`}
          <div class="cal-bday-age">Turning ${age}</div>
        </div>
      </div>`;
  }).join('');
  ...
  wrap.innerHTML = `
    ...
    <div class="cal-legend">
      <div class="cal-legend-item"><span class="cal-tag-dot work"></span>Work</div>
      <div class="cal-legend-item"><span class="cal-tag-dot social"></span>Friends</div>
      <div class="cal-legend-item"><span class="cal-tag-dot exam"></span>Exam</div>
    </div>
    <div class="cal-section-title">Upcoming Birthdays</div>
